...
const MapSettingsPanel = function(props){
  ...
  return h('div', [
    ...
    h(Switch, {
      label: "Keep north up",
      checked: keepNorthUp,
      onChange() { return updateState({$toggle: ['keepNorthUp']}); },
...
const ControlPanel = function(props){
  const {setTime, setModel, models, featureDatasets, featureDataset, setFeatureDataset} = props;
